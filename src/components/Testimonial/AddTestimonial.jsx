...
function AddTestimonial() {
  ...
  const handleFileChange = (e) => {
    setFormData({ ...formData, testimonialPic: e.target.files[0] });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.testimonialPic) {
      formDataToSubmit.append('testimonialPic', formData.testimonialPic);
    }

    try {
      const response = await axiosClients.post('/testimonial', formDataToSubmit, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
      toast.success('Testimonial created successfully.');
      navigate('/alltestimonialpage');
    } catch (error) {
