...
import ViewShowcasebox from './ViewShowcasebox'
import EditShowcasebox from './EditShowcasebox'
import DeleteShowcasebox from './DeleteShowcasebox'
import { axiosClients } from '../../Apis/api'

function AllShowcasebox() {
  ...
  const fetchData = async () => {
    try {
      const response = await axiosClients.get('/getAllShowCaseBox')
  ...
  const handleNextImage = () => {
    if (selectedImageIndex < selectedShowcasebox.image.length - 1) {
      setSelectedImageIndex(selectedImageIndex + 1)
    }
  }
  ...
  return (
    <div className="container-fluid">
          ...
          <div className="card card-table">
            <div className="card-body">
              <div className="title-header option-title d-sm-flex d-block">
                <h5>Showcasebox List</h5>
                <div className="right-options">
                  <ul>
                    <li>
                      <Link className="btn btn-dashed" to="/addshowcasebox">
                        Add Showcasebox
                      </Link>
                    </li>
                  </ul>
              ...
              <div className="table-responsive">
                <table className="table category-table dataTable no-footer" id="table_id">
                  ...
                  <tbody>
                    {currentBanner?.map((showcasebox, i) => (
                      <tr key={i}>
                        <td>{indexOfFirstAd + i + 1}</td>
                        <td>
                          <img
                            src={showcasebox.image}
                            alt={showcasebox.heading}
                            onClick={() => {
                              setSelectedShowcasebox(showcasebox)
                        ...
                        <td>{showcasebox.heading}</td>
                        <td>{showcasebox.title}</td>
                        <td>
                          {showcasebox.link ? showcasebox.link.substring(0, 15) + '...' : 'N/A'}
                        </td>
                        <td>
                          <span
                            className={`badge ${showcasebox.status === 'active' ? 'bg-success' : 'bg-danger'}`}
                          >
                            {showcasebox.status}
                          </span>
                        </td>
                        <td>
                          <ul className="d-flex gap-3">
                            <li onClick={() => handleViewBanner(showcasebox)}>
                              <a className="text-warning">
                                <i className="ri-eye-line"></i>
                              </a>
                            </li>
                            <li onClick={() => handleEditModal(showcasebox)}>
                              <a>
                                <i className="ri-pencil-line text-success"></i>
                              </a>
                            </li>
                            <li onClick={() => deleteToggle(showcasebox)}>
                              <a>
      ...
      {selectedShowcasebox && (
        <>
          <ViewShowcasebox modal={modal} toggle={toggleModal} data={selectedShowcasebox} />
          <EditShowcasebox
      ...
      {selectedShowcasebox && selectedImageIndex !== null && (
        <Modal isOpen={zoomModal} toggle={toggleZoomModal} centered>
          <ModalBody>
            <img
              src={selectedShowcasebox.image} // Display only the selected testimonial image
              alt="Zoomed"
              style={{
                width: '100%',
                height: 'auto',
          ...
          <ModalFooter>
            ...
            <Button
              ...
              disabled={selectedImageIndex === selectedShowcasebox.image.length - 1}
            >
